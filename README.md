...
`product-technologist` and `qa-engineer` run inside the wrapper's process, without Assignment v1 or Result v1. `qa-engineer` is the project's gate: after development and before review the wrapper states the workspace, the base ref and the run directory and invokes the skill; the role finds the gate's commands and executor in the repository rules, proves the executor sees this workspace, runs every check through `qa-run.mjs` / `checks-run.mjs` (bundled in `skills/qa-engineer/scripts/`), writes a result file and hands back its path with the verdict — `green`, `red` (the change goes back to the developer with the failed items) or `unconfirmed` (review proceeds, delivery needs a human decision). `product-technologist` is the interactive one: it runs inside the wrapper's process. The wrapper loads the issue material, states a brief (the target or the fields for a new issue, the user's words, whether a document exists, the issue's size, the repository root, the project's terminology document), and invokes the skill; the role interviews the human round by round, researches the repository through read-only children, and finishes with the document, the issue title, and the list of mockup files in-context. The wrapper persists the document and the title, attaches the mockups to the issue, and moves the state.
...
The developer and reviewer share [project knowledge discovery, storage and counterexample checks](references/engineering-evidence.md). They assume only `docs/`, follow existing project documentation, and discover validation commands from executable configuration. A check runs on the paths the change touched by default; the shared reference names the cases that call for full width, and a narrowed green is evidence about the paths it covered and nothing else. The default location for new durable rules is `docs/engineering/`, created only for task-relevant knowledge; existing documents remain canonical.

The reviewer starts from one `review-pack.mjs` call (bundled in `skills/code-reviewer/scripts/`) that assembles the task text, the accepted decisions, the repository instruction chain (entry files whole, the documents they name as routes), risk signals, the check commands already carrying the diff's paths, and the code diff from `repository.base_ref` (test files ride as a digest of their declared cases, so the review judges the code and opens a test only on a concrete doubt) into a pack file outside the repository (in children mode also a `-lens` twin without the environment snapshot, so the lenses collect nothing of their own), then judges the change through three lenses — behavior, design, comments — in one pass over that single reading, delegating them to child agents only when the change did not fit the pack, and refutes every finding against the cited code before handoff. The project's gate runs once, in the QA role, and its result rides into the review packet as the `qa_result` material (the pack carries it whole; a packet without it is `blocked`): the reviewer runs no check of its own, a failed item is a finding, a broken one residual risk. The developer runs only the tests of the behavior it changed, through the executor the repository rules declare, and never a code-quality tool. A repeat review runs on the fix delta when the wrapper moves `base_ref` to the reviewed revision and passes the previous review. Both roles map acceptance scenarios to evidence in `deliverable.content.coverage`; reviewer coverage also accounts for rules and every changed file, `lenses` and `lenses_mode` record how the review ran, and supplied fixes are tracked in `deliverable.content.fix_resolution`. Every launched role receives the Assignment inline or as the file the launch names (`@<path>`), and a `source_materials` entry named `issue` with the task text — inline, or the path of the file the wrapper wrote the tracker text to, so no model retypes it; a review packet also carries `repository.base_ref`; without them the role returns `blocked`. The `issue` material is the product authority: its managed PRD block (`# Проблематика` / `# Продуктовое решение`) is the accepted decision, its «Критерии приемки» the acceptance scenarios, its `### Scope` the paths and `Эталон:` analogues; `accepted_decisions` and `verification` add only what the wrapper knows beyond it. [`contracts/validate-assignment.mjs`](contracts/validate-assignment.mjs) is the executable contract a wrapper runs on the packet before every launch; `review-pack.mjs --assignment <path> --check` adds base and diff resolution for a review. A role writes its full Result v1 to `result-<assignment_id>.json` beside the packet and returns only the envelope (decision fields, fix IDs, the file path); [`contracts/validate-result.mjs`](contracts/validate-result.mjs) validates the file and prints that envelope, so the wrapper routes on it and forwards the file — the reviewer's Result to the developer as the `review` material and to the next reviewer as `previous_review`, the developer's to the reviewer as `development_result` and into the commit body. Assignment v1 and Result v1 schemas remain unchanged.
