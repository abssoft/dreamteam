...
The reviewer starts from one `review-pack.mjs` call (bundled in `skills/code-reviewer/scripts/`) that assembles the task text, the accepted decisions, the project rules, risk signals and the whole diff from `repository.base_ref` into a pack file outside the repository, then judges the change through three lenses — behavior, design, comments — as child agents for a large or risky change and in its own context otherwise, and refutes every finding against the cited code before handoff. Both roles map acceptance scenarios to evidence in `deliverable.content.coverage`; reviewer coverage also accounts for rules and every changed file, `lenses` and `lenses_mode` record how the review ran, and supplied fixes are tracked in `deliverable.content.fix_resolution`. A review packet must carry `repository.base_ref` and the task text as a `source_materials` entry named `issue` (a subtask also `parent_issue`); without them the reviewer returns `blocked`. Assignment v1 and Result v1 schemas remain unchanged.
