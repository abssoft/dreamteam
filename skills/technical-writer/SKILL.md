...
Require `contract_version: 1`, `role: technical-writer`, accepted product and technical decisions, exact scope, audience and purpose, target path or section, repository context with current `head_sha`, changed-file evidence, verification results, permissions, and `return_contract: result-v1`.
...
1. Inspect the actual changed surface, accepted decisions, source evidence, and relevant existing documentation.
