...
`ok: true` → `label`, `wall_seconds` (active time: idle gaps over 30 minutes between records — e.g. a session resumed the next day — are excluded; `ended_at - started_at` still spans the calendar), `started_at`, `ended_at`, `agents`, `steps`, `models`, `tokens {input, cached_input, output, total}`, `cost_usd`, `unpriced_models`, `by_model` (per-model `wall_seconds`/`steps`/`tokens`/`cost_usd`, `cost_usd: null` when the model has no price entry), `source`, and two ready-to-paste renderings:
