...
- `reviewer-seeded-defect` — the change carries a real defect the bundled test suite does not catch (green tests, wrong behavior). Passing requires a non-empty `required_fixes` naming the defect: it proves the reviewer reads the diff instead of trusting the green gate in `qa_result`.
